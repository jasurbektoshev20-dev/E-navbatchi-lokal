            ...
            <div class="card">
                <div class="card-datatable table-responsive">
                    <table class="datatables-projects table border-top">
                        ...
                        <tbody>
                            {foreach from=$Structures item=Table key=tkey}
                                <tr class="lb" id="row_{$Table.id|crypt}">
                                    <td>{$tkey+1}</td>
                                    <td><a href="hr.php?act=cooperate_duty&mid={$smarty.get.mid}&id={$Table.id|crypt}">{$Table.name1}</a></td>
                                    <td>{$Table.name2}</td>
                                    <td>{$Table.name3}</td>
                                    <td>{$Table.phone}</td>
                                    <td>{$Table.comment}</td>
                                    <td class="text-center">
                                        {if $Table.logo neq ""}
                                            <img src="/pictures/cooperates/{$Table.logo}" height="50" class="Thumb" >
                                        {else}
                                            <img src="/assets/images/no_img.png" height="28" class="Thumb">
                                        {/if}
                                    </td>
                                    <td>
                                        <div class="dropdown">
            ...
            <div class="modal-body">
                ...
                <form class="needs-validation" novalidate>
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <label for="phone" class="form-label">{$Dict.phone}</label>
                            <input type="text" class="form-control" id="phone" />
                        </div>
                        <div class="col-12">
                            <label for="comment" class="form-label">{$Dict.comment}</label>
                            <input type="text" class="form-control" id="comment" />
                        </div>
                        <div class="col-12">
                            <label class="form-label">{$Dict.choose_file}</label>
                            <div action="/upload" class="dropzone needsclick" id="logo">
...
<script>
    ...
    {literal}
        ...
        const dropzoneBasic = document.querySelector('#logo');
        ...
        $('.datatables-projects tbody').on('click', '.delete', function() {
            ...
            $.get("hrajax.php?act=del_cooperate&rowid=" + RowId, function(html) {
                if (html == 0) {
                    $("#row_" + RowId).remove();
                }
            });
        ...
        $('.datatables-projects tbody').on('click', '.editAction', function() {
            ...
            var RowId = $(this).attr('rel');
            $.get("hrajax.php?act=get_cooperate&rowid=" + RowId, function(html) {
        ...
        Array.prototype.slice.call(bsValidationForms).forEach(function (form) {
            form.addEventListener('submit', function (event) {
                if (!form.checkValidity()) {
                    ...
                    $.ajax({
                        url: 'hrajax.php?act=act_cooperate',
                        dataType: 'text',
                        cache: false,
                        contentType: false,
                        processData: false,
