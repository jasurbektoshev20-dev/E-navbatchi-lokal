            ...
            <div class="card">
                <div class="card-datatable table-responsive">
                    <table class="datatables-projects table border-top">
                        ...
                        <tbody>
                            {foreach from=$HrTechGuardCars item=Table key=tkey}
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <select required class="form-control" name="car_model_id" id="car_model_id">
                                ...
                                {foreach from=$RefCarModels item=Item3 key=ikey3}
                                    <option value="{$Item3.id}">{$Item3.name}</option>
                                {/foreach}
                            </select>
                        </div>
                        <div class="col-12">
                            <label>{$Dict.state_number}</label>
                            <input type="text" class="form-control" name="plate_number" id="plate_number" value="">
                        </div>
                        <div class="col-12">
...
<script>
    ...
    {literal}
        ...
        $('.datatables-projects tbody').on('click', '.delete', function() {
            ...
            $.get("hrajax.php?act=del_tech_guard_cars&rowid=" + RowId, function(html) {
                if (html == 0) {
                    $("#row_" + RowId).remove();
                }
            });
        ...
        $('.datatables-projects tbody').on('click', '.editAction', function() {
            ...
            var RowId = $(this).attr('rel');
            $.get("hrajax.php?act=get_tech_guard_cars&rowid=" + RowId, function(html) {
                var sInfo = jQuery.parseJSON(html);

                $('#structure_id').val(sInfo.structure_id);
                $('#structure_id').trigger("change");
                $('#car_model_id').val(sInfo.car_model_id);
                $('#car_model_id').trigger("change");
        ...
        Array.prototype.slice.call(bsValidationForms).forEach(function(form) {
            form.addEventListener('submit', function(event) {
                if (!form.checkValidity()) {
                    ...
                    $.ajax({
                        url: 'hrajax.php?act=act_tech_guard_cars',
                        dataType: 'text',
                        cache: false,
                        contentType: false,
                        processData: false,
